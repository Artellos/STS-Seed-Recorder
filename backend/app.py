...
# ── Auto-generate maps from seed (pure Python) ───────────────────────────────

@app.route("/api/sts2/status", methods=["GET"])
def sts2_status():
    """Always available: uses pure Python map generation."""
    return jsonify({"available": True, "message": "Pure Python map generator ready"})


@app.route("/api/seeds/<int:seed_id>/generate", methods=["POST"])
def generate_map(seed_id):
    """
    Generate all act maps from the seed using the pure Python algorithm and
    ...
    """
    from map_generator import generate_all_maps
    ...
    try:
        maps = generate_all_maps(seed_value)
    except Exception as e:
    ...
    with get_connection() as conn:
        ...
        coord_to_id: dict = {}
        for act_map in maps:
            act_num = act_map["act_index"] + 1
            for node in act_map["nodes"]:
                row = conn.execute(
                    "INSERT OR IGNORE INTO nodes "
                    "(seed_id, act, floor, col, node_type) VALUES (?,?,?,?,?)",
                    (seed_id, act_num, node["row"], node["col"], node["type"]),
                ).fetchone()
                # Fetch the inserted / existing id
                existing = conn.execute(
                    "SELECT id FROM nodes WHERE seed_id=? AND act=? AND floor=? AND col=?",
                    (seed_id, act_num, node["row"], node["col"]),
                ).fetchone()
                if existing:
                    coord_to_id[(act_num, node["row"], node["col"])] = existing["id"]

        for act_map in maps:
            act_num = act_map["act_index"] + 1
            for conn_data in act_map["connections"]:
                from_id = coord_to_id.get((act_num, conn_data["from_row"], conn_data["from_col"]))
                to_id   = coord_to_id.get((act_num, conn_data["to_row"],   conn_data["to_col"]))
    ...
    counts = {f"act{m['act_index']+1}": len(m["nodes"]) for m in maps}
    return jsonify({
        "status": "done",
        "progress": "Complete",
        "error": None,
...
def generate_status(seed_id):
    """
    Kept for frontend compatibility. Generation is now synchronous so this
    always returns idle unless called before the POST completes (race).
    """
